    ...
    )
{
  ...
  for (unsigned int t = 0; t < T; ++t)
    logqYj.col(t) = (Njik * sweights[t].Elogweight().matrix()).array()
        + E_logwj(t) + iclusters[t].Eloglike(Oj).array();
...
template <class IW, class SW, class IC, class SC> double fenergy (
    ...
    const vector<IC>& iclusters, // Image cluster parameters
    const vector<SC>& sclusters, // Segment cluster parameters
    const double Fy ,            // Free energy Y terms
...
template<class IW, class SW, class IC, class SC> double vbem (
    const vMatrixXd& O,           // Image observations
    ...
    )
{
  ...
  iclusters.resize(T, IC(iclusterprior, O[0].cols()));
  ...
  do
  {
    ...
    for (unsigned int j = 0; j < J; ++j)
    {
      // Image clusters
      for (unsigned int t = 0; t < T; ++t)
        iclusters[t].addobs(qY[j].col(t), O[j]);
...
//    #pragma omp parallel for schedule(guided)
    ...
    for (unsigned int j = 0; j < J; ++j)
      Fy += vbeY<IW,SW,IC>(O[j], qZ[j], iweights[j], sweights, iclusters,
                           qY[j]);
...
//      #pragma omp parallel for schedule(guided) reduction(+ : Fz)
    ...
    F = fenergy<IW,SW,IC,SC>(iweights, sweights, iclusters, sclusters, Fy, Fz);
...
//  #pragma omp parallel for schedule(guided)
  for (unsigned int j = 0; j < J; ++j)
  {
    ...
    MatrixXd Njik(X[j].size(), K);

    // Get segment cluster counts per image
    for (unsigned int i = 0; i < X[j].size(); ++i)
      Njik.row(i) = qZ[j][i].colwise().sum();

    // Add in cluster log-likelihood, weighted by responsability
    for (unsigned int t = 0; t < T; ++t)
    {
      double LL = qY[j].col(t).dot((logpi(t)
                            + (Njik * sweights[t].Elogweight().matrix()).array()
                            + iclusters[t].Eloglike(O[j]).array()).matrix());
...
//    #pragma omp parallel for schedule(guided) reduction(+ : Mtot, scount)
    for (unsigned int j = 0; j < J; ++j)
    {
      ...
      mapidx[j] = partobs(O[j], partind, Ot[j]);  // Copy :-(
...
//    #pragma omp parallel for schedule(guided)
    ...
    double Fsplit = vbem<IW,SW,IC,SC>(O, X, qYaug, qZaug, iwspl, swspl, icspl,
 ...
 *    throws: runtime_error from its internal VBEM calls
 */
//template <class IW, class SW, class SC> bool ssplit (
//    const vvMatrixXd& X,            // Observations
//    const vector<SC>& clusters,     // Cluster Distributions
    ...
    )
{
  ...
  if (O.size() != X.size()) // Same number of groups in observations
    throw invalid_argument("O and X need to have the same number of groups!");

  const unsigned int J = O.size();

  for (unsigned int j = 0; j < J; ++j) // Same number of images/docs in groups
    if ((unsigned) O[j].rows() != X[j].size())
      throw invalid_argument("O and X need to have the same number of 'docs'!");

  // TEMP: Randomly initialise qY and initialise qZ to ones
  qY.resize(J);
  qZ.resize(J);
...
//    ArrayXXd randm = (ArrayXXd::Random(Ij, 2)).abs();
...
//    qY[j] = (randm.log().colwise() - norm.log()).exp();
    ...
    for (unsigned int i = 0; i < Ij; ++i)
    {
      ArrayXXd randm = (ArrayXXd::Random(X[j][i].rows(), 3)).abs();
      ArrayXd  norm  = randm.rowwise().sum();
      qZ[j][i] = (randm.log().colwise() - norm.log()).exp();
    }
  ...
  while (issplit == true)
  {
    ...
    F = vbem<IW,SW,IC,SC>(O, X, qY, qZ, iweights, sweights, iclusters,
    ...
    issplit = isplit<IW,SW,IC,SC>(O, X, iweights, sweights, iclusters,
    ...
    )
{
  ...
  double F = mcluster<GDirichlet, Dirichlet, GaussWish, GaussWish>(O, X, qY, qZ,
